...
const validateResident = (blur) => {
    if (!formData.value.isAustralian && !formData.value.reason.trim()) {
        if (blur) errors.value.resident = "Non-residents must provide a reason for joining.";
    } else {
...
const validateReason = (blur) => {
    ...
    if (reason.length === 0) {
        ...
        errors.value.reason = `Reason must be no more than ${maxLength} characters long.`;
    } else {
        errors.value.reason = null;
    }
